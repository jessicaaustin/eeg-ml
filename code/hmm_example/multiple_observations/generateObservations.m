function [observations, hiddenStates] = generateObservations(n)
...
num_b = 2;
b = zeros(3,3,num_b);
% sensor 1: barometric pressure
     % low  medium  high pressure reading
b(:,:,1) = [.7    .2      .1;  % rainy
            .3    .5      .2;  % cloudy
            .1    .4      .5]; % sunny
% sensor 2: thermometer
     % low  medium  high temperature reading
b(:,:,2) =  [.5    .3      .2;  % rainy
             .4    .4      .2;  % cloudy
             .1    .4      .5]; % sunny
b
...
observations = zeros(n,num_b);
for i=1:n
    sensor_probabilities_for_state = b(states(i),:,:);
    for j=1:num_b
        observations(i,j) = discreteSample(sensor_probabilities_for_state(:,:,j),1);
    end
end
...
% [observations, hiddenStates] = sample_dhmm(p, A, b, 1, n);  

%% Sample using the MATLAB implementation
% [observations, hiddenStates] = hmmgenerate(n,A,b);
