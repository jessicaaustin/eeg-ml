...
if w==0 || w==-1
    ...
    %fprintf('UNWEIGHTED COHEN''S KAPPA\n')
    %fprintf('%s\n', tr)
    k = kcomp;
end
if w==1 || w==-1
    ...
    %fprintf('LINEAR WEIGHTED COHEN''S KAPPA\n')
    %fprintf('%s\n', tr)
    k = kcomp;
end
if w==2 || w==-1
    ...
    %fprintf('QUADRATIC WEIGHTED COHEN''S KAPPA\n')
    %fprintf('%s\n', tr)
    k = kcomp;
end
return
...
elseif k>=0 && k<=0.2
    %fprintf('Slight agreement\n')
elseif k>=0.21 && k<=0.4
    %fprintf('Fair agreement\n')
elseif k>=0.41 && k<=0.6
    %fprintf('Moderate agreement\n')
elseif k>=0.61 && k<=0.8
    %fprintf('Substantial agreement\n')
elseif k>=0.81 && k<=1
    %fprintf('Perfect agreement\n')
end
%fprintf('Variance = %0.4f     z (k/sqrt(var)) = %0.4f    p = %0.4f\n',var,z,p)
if p<0.05
    %fprintf('Reject null hypotesis: observed agreement is not accidental\n')
else
    %fprintf('Accept null hypotesis: observed agreement is accidental\n')
end
return
end
